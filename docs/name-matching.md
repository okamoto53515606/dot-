...
- 同じ jev を使った別ドメインの実装は [入力モデレーション（moderation.md）](./moderation.md) を参照
...
**エラーも認証画面も出しません。** 判定できないときに何もしないのが最も安全で、UX も壊しません。
利用者が止められることはなく、注文は常に成立します。
...
エンドポイントは `POST https://api.typesafe.ai/v1/systemone`、認証は `Authorization: Bearer $TYPESAFE_API_KEY` です。
下が実際に送るリクエストの全体です（`state` と `questions` の両方を含む）。
...
```json
{
  ...
  "questions": {
    ...
    "same_surname": {
      "type": "noul",
      "instructions": "Do `customer_a` and `customer_b` state the same surname (姓), allowing for kana/kanji variants and an old surname?"
    },
    "same_given_name": {
      "type": "noul",
      "instructions": "Do `customer_a` and `customer_b` state the same given name (名), allowing for kana/kanji variants and common spelling variants of the same reading?"
    },
    "same_phone": {
      "type": "noul",
      "instructions": "Do `customer_a` and `customer_b` state the same phone number, allowing for hyphens, spaces, and full-width digits?"
    },
    "same_address": {
      "type": "noul",
      "instructions": "Do `customer_a` and `customer_b` state the same address, allowing for formatting differences such as arabic numerals versus kanji numerals, block notation, and a prefecture that is omitted on one side?"
    },
    "same_birth_date": {
      "type": "noul",
      "instructions": "Do `customer_a` and `customer_b` state the same date of birth, allowing for different date formats?"
    }
  }
}
```
...
既存注文と今回の注文を 1 つの `state` に入れ、**ペア**に対する質問にします。

| フィールド | 内容 | 補足 |
|------------|------|------|
| `customer_a` | 比較対象（**直近の注文**） | §4.4 |
| `customer_b` | 今回のゲスト注文 | |
| `surname` / `given_name` | 姓と名を**分けて**持つ | 「姓だけ一致」と「フルネーム一致」を区別させるため |
| `phone` / `address` / `birth_date` | そのまま（**正規化しない**） | 表記ゆれの吸収は jev の役割。事前に正規化するとその能力を使えない（`2-8-1` と `二丁目8番1号` を同一と判定できる） |
| メールアドレス | **含めない** | §2.3 |
| ハッシュ化した値 | **含めない** | jev はテキストの類似を判断するモデルなので、ハッシュ値では判定できない。ブロッキングにハッシュを使っていても、判定時は**原文**を渡す |

#### `model` — 使用モデル

`jev-latest`。閾値をチューニングしたら `jev-1.13.0` のように**バージョンを固定**してください（§6 P8）。

#### `questions` — 質問

判定に使うのは `identity` の 1 本だけです。残り 5 本の Noul は同じリクエストに同乗させて、**どの項目が食い違っているかの記録**（ログ・事後検証用）に使います。
jev は同一リクエスト内の質問を並列評価するため、質問を増やしてもレイテンシはほとんど増えず、コストは質問トークン分だけです。

| ID | 型 | 問い | 使途 |
|----|----|------|------|
| `identity` | Score 3 レベル | 2 つのレコードは同一人物か | **フラグ判定に使う唯一の質問** |
| `same_surname` | Noul | 姓は同じか（カナ/漢字・旧姓を許容） | ログ・事後検証 |
| `same_given_name` | Noul | 名は同じか（カナ/漢字・表記ゆれを許容） | ログ・事後検証 |
| `same_phone` | Noul | 電話番号は同じか（ハイフン・全角を許容） | ログ・事後検証 |
| `same_address` | Noul | 住所は同じか（数字表記・都道府県省略を許容） | ログ・事後検証 |
| `same_birth_date` | Noul | 生年月日は同じか（日付形式を許容） | ログ・事後検証 |

`identity` の 3 レベルは、**レベル 0・1 が「フラグなし」、レベル 2 が「フラグあり」**に対応します（§2.4 の条件を満たすときだけフラグを立てる）。
...
結果として jev は「**個人属性だけを見て同一人物かどうかを判断**」します。実測でも、同姓同名の別人（D01）や全項目が異なるケース（D05）は score 0.05 / 0.00 でフラグが立ちません。
...
**大量処理の上限。** TypeSafe のレート制限は 1,200 リクエスト/分 = 20 リクエスト/秒なので、
1 ペア 1 リクエストでは**約 7 万ペア/時間**が上限になります。
1 リクエストに複数のペアを詰める（`state` にペアを並べ、`pair_0`〜`pair_n` の質問に分ける）ことで、
スループットとコストを同時に改善できます。制約は「`state` + 最長質問で 32k トークン」です。
...
> 「住所未登録」は本来発生しないケースです（住所は必須）。**防御的なテスト**として残しています。
...
- **FP 方向**: 家族で共有しているメール、使い捨てメール、入力ミス。
  属性判定で大半は落とせますが、**属性まで一致する家族は落とせません**（§4.1）。

実測でも、兄弟（D03: 姓・住所・固定電話が一致、名と生年月日が違う）や家族でのメール共有（M03）はフラグが立ちませんでした。
...
この設計は次の前提の上に成り立っています。**前提が変わると設計も変わります。**

| # | 前提 | 前提が崩れた場合の要注意事項 |
|---|------|------------------------------|
| P1 | **顧客マスタを持たず、マージもアカウント付与もしない。** 注文データにフラグを立てるだけ | 顧客データのマージやログイン状態の付与を伴う設計にする場合は、**属性一致は認証の代わりになりません**。姓・名・生年月日・住所・電話は漏泄データや SNS から入手でき、知識ベース認証（KBA）の強度しかありません。メール宛の確認（ワンタイムリンク等）を併用してください |
| P2 | **判断が曖昧・別人・jev 障害のときは何もしない。** エラーも認証画面も出さない | 「このメールアドレスはすでに利用されています」のようなエラーを出す設計にする場合は、**メールアドレスの存在確認（enumeration）に使われます**。レート制限・CAPTCHA・中立な文言などを併用してください。認証画面を出す設計にする場合は、**パスワードを持たないゲストの救済（リセット導線）**が必須です |
| P3 | **用途は不正注文チェック。** 誤フラグ（不正を見逃す）が重く、取りこぼし（新規扱い）は軽い | 用途が**クーポン等の特典配布**なら誤フラグが直接コストになるため、さらに厳しい閾値が必要です。**分析・集計のみ**なら誤フラグの実害は小さく、逆に閾値を緩めて再現率を上げる判断もあり得ます |
| P4 | **住所は必須（物販 EC）、生年月日も必須（酒類の年齢確認）。** 判定材料が常に揃う | 項目が欠落しうる設計（任意入力・B2B 顧客・越境 EC など）にする場合は、**欠落ペアの扱いを決める必要があります**。実測では住所未登録でも `score` が 1.63 まで上がるため、**必須項目が揃っていることを自動判定の前提条件にする**（コード側のガード）ことを検討してください |
| P5 | **一次絞り込みはメール完全一致のみ。** 比較対象は直近の注文 | 電話や「姓名＋生年月日」でも候補を拾う場合は、**候補ペア数＝コストが増えます**（1 ペア約 $0.000033）。比較対象を直近以外にする場合は、どれを比較するかを先に決めてください。**ブロッキングキーは「表記ゆれが起きにくい粒度」を選ぶ**のが原則です（下表） |
| P6 | **POC では実データを流さず、合成データのみで評価している** | 実データを流す場合は、**個人情報保護法 28 条（外国にある第三者への提供）・委託先の監督・DPA / ZDR 契約**の確認が前提になります |
| P7 | **推論は日本国内で閉じた環境で行う**（Bedrock 東京リージョン、またはオープンウェイトのセルフホスト） | 海外の API を直接使う構成にする場合は P6 の越境整理が必要です。また**プロバイダを変えると精度が変わる**ため、同じ 30 パターンで再評価してください（§5.2） |
| P8 | **バックフィルしない。** 今後の新規注文のみを判定する | 過去の注文にも遡ってフラグを立てる場合は、候補ペア数の見積もりと、再実行可能なバッチの設計が必要です |
| P9 | **判定材料はテキスト（姓・名・電話・住所・生年月日）のみ。** 配送先・決済・デバイスは見ていない | 誤フラグの実害をさらに下げたい場合は、**配送先の一致をコードで確認**するのが安価で効果的です（不正者は受け取りのため配送先を自分にするので、「属性一致・配送先不一致」は高リスクのシグナルになります） |

### 6.1 補足: ブロッキングキーの粒度（P5 でキーを増やす場合）

「完全一致で絞る」と「表記ゆれを拾う」は**トレードオフ**です。絞り込みに使うキーほど、揺れの少ない項目を選んでください。

| 項目 | ブロッキングに使う形 | 注意 |
|------|----------------------|------|
| メール | 小文字化 + トリムして完全一致（現在のキー） | 揺れが少なく強い |
| 電話 | **数字のみに正規化**して完全一致（`090-1234-5678` → `09012345678`） | 正規化すれば完全一致でよい |
| 住所 | **郵便番号**、または**都道府県＋市区町村＋町名（番地を除く）** | 番地・建物名まで完全一致を要求すると表記ゆれを取りこぼす。番地の同一性判断は jev に任せる |
| 生年月日 | 正規化して完全一致（補助キー） | 単独では別人を大量に拾う |

弱いキーの組み合わせは OR 条件で用意し、**再現率は候補生成で確保し、精度は jev で担保する**という分担にしてください。
